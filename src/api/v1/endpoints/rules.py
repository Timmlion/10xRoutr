...
try:
    ...
    templates = None
    print("[WARNING] Jinja2Templates instance 'templates' not found in main.py. HTMX responses might not work for List Rules.")
...
async def create_rule_endpoint(
    ...
    print(f"Received request to create rule for link {link_id} by user {current_user_id}")
    try:
        ...
        print(f"[NOT FOUND] Parent link {link_id} not found or access denied for user {current_user_id}: {e.detail}")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Parent link not found or access denied.")
        ...
        print(f"[ERROR] Unexpected error in create_rule_endpoint for link {link_id}: {e}")
        traceback.print_exc()
...
async def list_rules_endpoint(
    ...
    print(f"Received request to list rules for link {link_id} by user {current_user_id}. HTMX={is_htmx_request}")
    try:
        ...
        if is_htmx_request:
            if templates:
                ...
                return templates.TemplateResponse(
                 ...
                 print("[ERROR] HTMX request received but templates are not configured or imported.")
                 raise HTTPException(status_code=501, detail="HTML templating for HTMX is not configured.")
        else:
        ...
        print(f"[ERROR] Unexpected error in list_rules_endpoint for link {link_id}: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An internal server error occurred.")
...
async def get_rule_endpoint(
    ...
    print(f"Received request to get rule {rule_id} for link {link_id} by user {current_user_id}")
    try:
        ...
        return rule
    except NotFoundException as e: # Obejmuje nieznalezienie reguły lub linku nadrzędnego (jeśli serwis tak zgłasza)
        print(f"[NOT FOUND] {e.detail} for rule {rule_id} on link {link_id}, user {current_user_id}")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=e.detail)
    except DatabaseException as e:
        print(f"[ERROR] Database error getting rule {rule_id} for link {link_id}: {e.detail}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Database error: {e.detail}")
    except Exception as e:
        print(f"[ERROR] Unexpected error getting rule {rule_id} for link {link_id}: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An internal server error occurred.")
...
async def update_rule_endpoint(
    ...
    update_data: RuleUpdate, # Ciało żądania
    current_user_id: UUID = Depends(get_current_user_id),
    rule_service: RuleService = Depends(get_rule_service)
):
    """
    ...
    print(f"Received request to update rule {rule_id} for link {link_id} by user {current_user_id} with data: {update_payload_info}")
    try:
        ...
        return updated_rule
    except NotFoundException as e: # Może być rzucony przez weryfikację własności lub nieznalezienie reguły
        print(f"[NOT FOUND] {e.detail} during update for rule {rule_id} on link {link_id}, user {current_user_id}")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=e.detail)
    except ValidationException as e: # Błąd walidacji spójności z serwisu
         print(f"[BAD REQUEST] Validation error during update for rule {rule_id}: {e.detail}")
         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=e.detail)
    except PriorityConflictException as e:
        print(f"[CONFLICT] Priority conflict during update for rule {rule_id} on link {link_id}: {e.detail}")
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=e.detail)
    except DatabaseException as e:
        print(f"[ERROR] Database error updating rule {rule_id} for link {link_id}: {e.detail}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Database error: {e.detail}")
    except ServiceException as e:
         print(f"[ERROR] Service error updating rule {rule_id} for link {link_id}: {e.detail}")
         raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=e.detail)
    except Exception as e:
        print(f"[ERROR] Unexpected error updating rule {rule_id} for link {link_id}: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An internal server error occurred.")
...
async def delete_rule_endpoint(
    ...
    """
    print(f"Received request to delete rule {rule_id} for link {link_id} by user {current_user_id}")
    try:
        ...
        print(f"[NOT FOUND] {e.detail} during delete for rule {rule_id} on link {link_id}, user {current_user_id}")
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=e.detail)
    except DatabaseException as e:
        print(f"[ERROR] Database error deleting rule {rule_id} for link {link_id}: {e.detail}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Database error: {e.detail}")
    except Exception as e:
        print(f"[ERROR] Unexpected error deleting rule {rule_id} for link {link_id}: {e}")
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An internal server error occurred.")