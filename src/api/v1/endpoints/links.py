...
# Import zależności
from src.api.deps import get_current_user_id, get_link_service, pagination_dependency # Używamy zależności
...
try:
    ...
    print("[WARNING] Jinja2Templates instance 'templates' not found in main.py. HTMX responses might not work.")
...
async def list_links_endpoint(
    ...
    pagination: PaginationParams = Depends(pagination_dependency), # Używamy modelu Pydantic
    ...
    print(f"Received request to list links for user {current_user_id} with pagination: page={pagination.page}, size={pagination.page_size}. HTMX={is_htmx_request}")
    ...
    try:
        ...
        if is_htmx_request:
            if templates:
                print(f"Rendering HTMX partial for {len(paginated_result.items)} links.")
                ...
                return templates.TemplateResponse(
                 ...
                 raise HTTPException(status_code=501, detail="HTML templating for HTMX is not configured on the server.")
        else:
...
async def create_link_endpoint(
    ...
    """
    print(f"Received request to create link with alias '{link_data.alias}' for user {current_user_id}")
    try:
...
async def update_link_endpoint(
    ...
    print(f"Received request to update link {link_id} for user {current_user_id} with data: {update_payload_info}")
    try:
        ...
        print(f"[NOT FOUND] {e.detail} during update for link {link_id}, user {current_user_id}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=e.detail
        )
    except ValidationException as e: # Obsługa błędu walidacji z serwisu
         print(f"[BAD REQUEST] Validation error during update for link {link_id}: {e.detail}")
         raise HTTPException(
...
async def delete_link_endpoint(
    ...
    try:
        ...
        print(f"[NOT FOUND] {e.detail} during delete for link {link_id}, user {current_user_id}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=e.detail
        )
...
async def get_link_statistics_endpoint(
    ...
    try:
        ...
        print(f"[NOT FOUND] {e.detail} for link stats {link_id}, user {current_user_id}")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=e.detail
        )
